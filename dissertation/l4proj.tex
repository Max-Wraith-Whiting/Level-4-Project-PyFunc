...
\clearpage
\begin{lstlisting}[language=Caml, breaklines=false, caption=The Abstract Syntax for PyFunc's IR written in OCaml.]
type tree = 
...
\subsection*{Multi-Parameter Functions and List Processing}

In the case of the IR, Multi-parameter functions and list processing are oddly similar processes, and especially so in the case of PyFunc's IR.
The IR is limited to single argument anonymous lambda function internally.
This means that a function is \emph{not} represented by a single lambda or even just a collection of lambdas.
...
\emph{So how does it work?}

For Hindley-Milner to make much sense as a type-system, the first thing to understand is how types work on the lambda calculus at all.
