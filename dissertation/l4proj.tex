...
\begin{abstract}
    ...
    % FPL performed in a [similar/dissimilar] manner to Python in the selected test-suites, though as a prototype, it could not compare for more advanced features.
...
% Instead, it's about combining existing an existing functional programming base with Python's user-friendly and efficient nature.
...
% WHY!
% The functional paradigm itself suffers for lack of novice programmers and its academic oeuvre.
...
% There is a complete guide on good citation practice by Peter Cox head available here: \url{http://www.cs.bham.ac.uk/~pxc/refs/index.html}. 
...
% However, despite its  advantages, Python is not without its critics.
...
% Functional programming argues for caution when using mutability and 
...
% Python is simple enough for anyone to get started in, but still powerful enough to handle large complex projects.
...
% Often times, the ability to construct a program fast, to iterate quickly, is far more valuable than the CPU time it incurs.

% The readability also play a significant part in the value of the language.
...
% These rules are given by a typing judgements (sometimes referred to as assertions), usually written in the form:
...
% \ref{eq:context-introduction} shows the relatively simple inference that if expression $e$ of type $\tau$ is a member of context $\Gamma$ then it can be inferred that context $\Gamma$ contains expression $e$ of type $\tau$.
...
\begin{table}[!h]
    ...
    \begin{tabular}{@{}|l|l|l|l|@{}}
        ...
        Application & <function><variable or function> & $(\lambda x . x) y$    & calling the function "$\lambda x . x$" with argument "y"\\
        \hline
    \end{tabular}}
\end{table}
...
% The dominant programming paradigm is imperative, no matter the up tick of interest in functional programming.
...
%     \item Limited resources and support
...
% The language is aimed to be largely expression-based, but the necessary inclusion of declarations in the form of function definitions adds a layer of complication.
...
% The project has little need for complex lexical analysis to carefully delineate scopes, so the use of curly braces to determine scope is not unreasonable to keep.
...
% The actual power of it is somewhat harder to quantify without examples.
...
% You sub-in values incrementally to compute expressions, and in the case of type-inference this means that as substitutions occur their polymorphic types get constrained repeatedly until they are monomorphic (or in some cases they remain polymorphic if the constraints never reach the point of monomorphic).
...
% Most programming languages approach type systems from the monomorphic perspective, where atomic types are outlined, and evermore complex constructions out of them build a type system.
% % (In some languages this is more explicit like C, where making  is in effect building new monomorphic types as composites of other atomic types.)
% % \footnote{As an interesting side note, union type  in C are not  to polymorphic types in this analogy, though they are only as polymorphic as they are explicitly made.}
...
% Now, typed-lambda calculus is all well and good, but there is a fundamental problem. Type inference in polymorphic lambda calculus is not . (In logic, in a true/false decision problem is only  if there exists an effective method for  the correct answer.)
...
% \textbf{Now say why HM is better. (Type inference without annotations. Complete. .)}
...
% \subsection{Type System Formalisms and Definitions [Might kill this sec]}
...
\begin{mathpar}
    \inferrule*[right=Let]
    {\Gamma \vdash e_0: \tau \; \Gamma, x : \mathit{generalise}(\tau) \vdash e_1 : \tau'}
...
%     A function with polytype $\forall \alpha . \alpha \rightarrow \alpha$ can map any value of the same type to itself, which is by  the identity function.
%     The way to check equality of polytypes is up to reordering the quantification and $\alpha$-conversion.
%     Further, quantified variables not  in the monotype can be dropped.
...
%     % In a type $\forall a_1 \dots \forall a_n \cdot \tau$, the $\forall$ is the  binding the type variables $a_i$ in the monotype $\tau$.
%     % The variables $a_i$ are called quantified and any  of a quantified type variable in $\tau$ is called \emph{bound}.
%     % The unbound variables are called \emph{free}.
%     % Type variables can be bound by  in the given context ($\Gamma$), but with the inverse effect on the right hand side of the $\vdash$.
...
%     Formally, in HM, a type $\sigma ' $ is more general than $\sigma$, if some quantified variable in $\sigma'$ is consistently  such that one gains $\sigma$.
...
%     Type  that arise from the above specified type-order are consistent in their replacement.
...
%         return any(occurs_check(var, sub-term) for sub-term in term's sub-terms)
%     else:
%         return false
% \end{lstlisting}
    ...
    % Type substitutions that arise from the above specified type-order are consistent in their replacement.
    % % \item Natural Deduction
    % \item Typing rules
    % \item Let Polymorphism
    % \item Generalisation
...
\caption{An interpretation of Robinson's Unification Algorithm \citep{Robinson_1965}, \citep{Luger}. This algorithm can handle type constructions such as functions $a \rightarrow a$ by breaking down their arguments. The type constructions themselves are referred to as $functors$.}
\end{algorithm}
\end{minipage}
\end{center}
...
% 0. An initial pass that converts white space indentation into explicit scopes for later parsing.
...
% At the bottom of the rule-set the series of buffer actions are simply the parsing of string literals into single contiguous tokens referring to the literal value of the string.
...
% When an  function returns an interpreted value

% Contrary to the  in the type checker which were only passed down the ast as an argument, the interpreter uses a reference type to handle the env.
...
% Beyond these  edge cases which required special attention, 
...
% You may include multiple figures in one float, as in Figure \ref{fig:synthetic}, using \texttt{}, which is enabled in the template.
...
%       %(or a blank line to force the  onto a new line)
%     \begin{subfigure}[b]{0.45\textwidth}
...
%     This is an example of using , with \texttt{}s in the caption.
%     }\label{fig:synthetic}
% \end{figure}
...
% \caption{The Metropolis-Hastings  algorithm for drawing samples from arbitrary probability distributions, 
...
%         s_table = np.zeros((16, 16, 16, 16), =np.uint8)
...
%         The implementation performed relatively well on an applicable subset of the CPython test suite, producing favourable results that indicate a level of functionality in the PyFunc implementation.
%         It should be noted that the CPython test-suite is not a standardised Python implementation test-suite, and should not be considered the definitive measure for Python implementations.
